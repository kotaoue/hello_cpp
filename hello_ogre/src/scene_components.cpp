...
CubeRotator::CubeRotator(Ogre::SceneNode *node) : mNode(node) {}

bool CubeRotator::frameRenderingQueued(const Ogre::FrameEvent &evt)
{
...
TetraRotator::TetraRotator(Ogre::SceneNode *node) : mNode(node) {}

bool TetraRotator::frameRenderingQueued(const Ogre::FrameEvent &evt)
{
...
BallOrbiter::BallOrbiter(Ogre::SceneNode *orbitNode, float angularSpeed)
    : mOrbitNode(orbitNode), mAngularSpeed(angularSpeed)
{
}

bool BallOrbiter::frameRenderingQueued(const Ogre::FrameEvent &evt)
{
    mOrbitNode->yaw(Ogre::Radian(evt.timeSinceLastFrame * mAngularSpeed));
    return true;
}

void SparkleTrail::spawnParticle(const Ogre::Vector3 &origin)
{
...
SparkleTrail::SparkleTrail(Ogre::SceneManager *scnMgr,
...
bool SparkleTrail::frameRenderingQueued(const Ogre::FrameEvent &evt)
{
...
void AuroraBackground::buildAurora(bool isUpdate)
{
    if (isUpdate)
...
AuroraBackground::AuroraBackground(Ogre::SceneManager *scnMgr, Ogre::SceneNode *parentNode)
...
bool AuroraBackground::frameRenderingQueued(const Ogre::FrameEvent &evt)
{
...
void BackgroundColorAnimator::chooseNextTarget()
{
    mStart = mCurrent;
...
BackgroundColorAnimator::BackgroundColorAnimator(Ogre::Viewport *viewport)
    : mViewport(viewport),
...
bool BackgroundColorAnimator::frameRenderingQueued(const Ogre::FrameEvent &evt)
{
...
CameraFocusAnimator::CameraFocusAnimator(Ogre::SceneNode *cameraNode, Ogre::SceneNode *targetNode)
    : mCameraNode(cameraNode), mTargetNode(targetNode), mTime(0.0f)
{
}

bool CameraFocusAnimator::frameRenderingQueued(const Ogre::FrameEvent &evt)
{
    mTime += evt.timeSinceLastFrame;
...
void createFieldCage(Ogre::SceneManager *scnMgr)
{
    ...
    fieldCage->end();
    Ogre::SceneNode *fieldNode = scnMgr->getRootSceneNode()->createChildSceneNode();
